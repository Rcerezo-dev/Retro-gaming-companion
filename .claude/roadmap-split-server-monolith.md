...
**Qué mover:**
- El bloque de variables de daemon (líneas 282–289): `_auto_sync_enabled`, `_auto_sync_last_devices`, `_auto_sync_status`, `_sd_sync_status` — ahora viven en `state.py` (Paso 1), así que solo hay que mover las funciones.
...
def start_all(config: AppConfig, repository: LibraryRepository) -> None:
    ...
    if config.auto_sync_enabled:
        ...
        t.start()
        _logger.info("Auto-sync daemon arrancado")
...
server.py (~180 líneas):
  ...
  _ES_PLATFORM_FOLDERS / _STANDARD_PLATFORM_FOLDERS — (mover a platforms.py en otra rama)
...
Eliminar en `server.py` los bloques ya migrados (estado global, auth, helpers, daemons). Los alias de compatibilidad (Paso 2) se pueden mantener o eliminar según si algún handler los referencia.
...
> Este paso es el más tedioso pero el más importante para eliminar el acoplamiento circular.

**Commit:** `refactor: handlers importan desde state.py en lugar de server.py`

**Verificación final:**
- `python -m rom_manager serve` arranca limpio
- `grep -r "import rom_manager.web.server" src/` devuelve solo `server.py` y `state.py`
