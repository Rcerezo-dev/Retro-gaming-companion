"""ES-DE detection plus assorted system/status/misc routes.

Registered onto the shared router by ``register_system``; the orchestrator in
``esde/__init__.py`` calls it. Covers local-url/status/test-path/list-drives,
setup-status, ES-DE detection + asset copy, disc folders, system-status,
cloud-folder detection, retroarch/bios checks, es_systems generation, n64 scan
and shutdown.

The ``_handle_*`` helpers are module-level (not nested in ``register_system``)
because they are also imported by ``server.py`` internals and re-exported from
the ``esde`` package via ``esde/__init__.py``.
"""
...
def register_system(
    ...
    *,
    ...
    """Register ES-DE, status, system and misc utility routes."""
