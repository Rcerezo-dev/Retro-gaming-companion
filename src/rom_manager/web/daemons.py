    ...
    while True:
        try:
            ...
            if pending and not _state._job_manager.get_status()["inbox_running"]:
                _logger.info("Inbox watcher: %d archivos detectados, lanzando pipeline", len(pending))
                ...
                def _watcher_run(_tr=target_root_str) -> None:
                    _run_inbox_pipeline(
                        config.inbox_path, _tr,
                        config.inbox_delete_source, repository, config,
                        _state._job_manager,
                    )
...
def start_all(config: AppConfig, repository: LibraryRepository) -> None:
    ...
    t_health.start()
    _logger.info("Health check scheduler arrancado (intervalo: %d días)", _HEALTH_CHECK_INTERVAL_DAYS)
