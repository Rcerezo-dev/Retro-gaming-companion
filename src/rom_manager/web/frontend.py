  ...
  <div style="background:#1a1a2e;border:1px solid #3a3a5c;border-radius:10px;width:100%;max-width:680px;box-shadow:0 8px 40px rgba(0,0,0,0.7);position:relative;overflow:hidden">
    ...
    <div id="wizard-page-1">
      <div style="padding:32px 36px 0">
        <div style="font-size:26px;font-weight:700;color:#c9bcf5;margin-bottom:6px">&#x1F3AE; Bienvenido a Retro Vault</div>
        <div style="font-size:14px;color:#888;margin-bottom:24px">Vamos a organizar tu biblioteca en unos minutos.</div>

        <!-- Auto-detect -->
        <div style="margin-bottom:16px;display:flex;justify-content:flex-end">
          <button id="wiz-detect-btn" class="btn" onclick="wizardAutoDetect()" style="font-size:12px;padding:5px 12px;background:#1e1e3a">
            &#x1F50D; Detectar autom&#xe1;ticamente
          </button>
        </div>
        <div id="wiz-detect-msg" style="display:none;font-size:12px;padding:8px 12px;border-radius:6px;margin-bottom:12px;border:1px solid #2a2a3c;background:#0d0d1a;color:#888"></div>
...
<div id="tab-games" class="tab">
  ...
  <div id="games-view-wrap">
    ...
    <div id="games-grid-view" style="display:none">
...
<div id="tab-duplicates" class="tab">
  ...
  <div style="margin-top:28px;border-top:1px solid #2a2a3e;padding-top:20px">
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:8px;flex-wrap:wrap">
      ...
      <button class="btn danger" id="btn-ra-dups-discard-all" onclick="discardAllRaDuplicates()" style="font-size:12px;padding:3px 10px;display:none" title="Elimina del disco todos los duplicados que no tienen logros en RetroAchievements, conservando los que sí los tienen. No se puede deshacer.">Eliminar todos sin logros</button>
    </div>
...
<div id="tab-sync" class="tab">
  ...
  <div class="actions-panel" style="margin-top:20px">
    ...
    <p style="color:#888;font-size:13px;margin:0 0 12px">Compara por SHA1 los ROMs de la BD de PC con los de la BD de Android.</p>
    <div class="actions-row">
  ...
  <div class="actions-panel" style="margin-top:20px">
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:4px">
      ...
      <button class="btn" id="btn-rclone-check" onclick="toggleRcloneSetup()" style="font-size:11px;padding:3px 12px">&#x2699; Verificar</button>
    </div>
    <p style="color:#555;font-size:12px;margin-bottom:0">Comprueba si rclone est&#xe1; instalado y qu&#xe9; remotes tienes configurados.</p>
    <div id="rclone-setup-panel" style="display:none;margin-top:14px">
      <div id="rclone-status-info" style="font-size:12px;color:#888;margin-bottom:12px;line-height:1.7">Comprobando&#x2026;</div>
      <div id="rclone-remotes-panel" style="display:none">
        <div style="margin-bottom:10px">
          <label style="font-size:12px;color:#aaa;display:block;margin-bottom:6px">Remote activo &#x2014; elige uno y configura la ruta</label>
          <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
            <select id="rclone-remote-select" style="flex:1;min-width:140px;background:#0f0f0f;border:1px solid #444;color:#d4d4d4;padding:6px 8px;border-radius:4px;font:inherit;font-size:12px"></select>
            <input id="rclone-path-input" type="text" style="flex:2;min-width:160px" placeholder="/RetroSync/saves">
            <button class="btn primary" onclick="applyRcloneRemote()" style="font-size:12px;padding:5px 14px;flex-shrink:0">Guardar</button>
          </div>
          <div style="font-size:11px;color:#555;margin-top:4px">El valor se guardar&#xe1; en <code>config.toml</code> como <code>sync.remote</code>.</div>
        </div>
...
<div id="tab-cable" class="tab">
  ...
  <div class="actions-panel" style="max-width:780px">
    ...
    <div style="margin-bottom:10px;display:flex;align-items:center;gap:10px;flex-wrap:wrap">
      <button class="btn" style="padding:5px 12px;font-size:12px;border-color:#569cd6;color:#569cd6" onclick="loadCableSyncPreview()">&#x1F50D; Ver resumen</button>
      <div id="cable-preview-result" class="cable-preview"></div>
    </div>
...
<div id="tab-tools" class="tab">
  ...
  <div class="actions-panel" style="margin-bottom:24px;border-left-color:#c586c0">
    ...
    <div class="actions-row" style="gap:20px;align-items:center;flex-wrap:wrap">
      <label class="fmt-check"><input type="checkbox" id="batch-scan" checked> Escanear</label>
      <label class="fmt-check"><input type="checkbox" id="batch-match" checked> Identificar (DAT)</label>
      <label class="fmt-check"><input type="checkbox" id="batch-zip"> Descomprimir ZIPs</label>
      <label class="fmt-check"><input type="checkbox" id="batch-chd"> Convertir a CHD</label>
      <label class="fmt-check"><input type="checkbox" id="batch-health"> Health Check</label>
      <label class="fmt-check"><input type="checkbox" id="batch-ra"> RetroAchievements</label>
    </div>
    <div class="actions-row" style="margin-top:10px">
...
<div id="tab-settings" class="tab">
  ...
  <div class="actions-panel" style="margin-top:20px">
    ...
    <div id="catalog-status" style="margin-bottom:12px;font-size:12px;color:#888">Cargando&#x2026;</div>
    <div class="actions-row" style="margin-bottom:6px">
      <input id="import-dats-folder" type="text" style="flex:1" placeholder="Carpeta con archivos .dat / .xml">
      <button class="btn" onclick="importDats()">Importar DATs</button>
    </div>
    <div id="import-dats-result" class="job-result"></div>
    <div style="margin-top:8px;font-size:11px;color:#555;line-height:1.8">
      Descarga DATs de <strong style="color:#888">dat.no-intro.org</strong> (requiere cuenta gratuita) y de
      <strong style="color:#888">redump.org</strong>. Pon los archivos en cualquier carpeta y usa el importador de arriba,
  ...
  <div class="actions-panel" style="margin-top:20px;max-width:640px">
    ...
    <p style="color:#555;font-size:12px;line-height:1.7;margin-bottom:12px">
      Para que Retro Vault arranque automaticamente al iniciar Windows (y el sync automatico siempre
