...
| PSX-STRUCTURE-2b | Investigar por qué solo 33 de los ~58 `.bin` esperados (60 reales − 2 ya con `.cue`) salieron como candidatos de `generate-cues` — puede que la cifra de "59 sanos" de la sesión anterior fuera aproximada, o que algunos de esos `.bin` ya no estén sueltos en la raíz de `psx/` (migrados a subcarpeta en otra tarea del mismo día). Revisar antes de dar `PSX-STRUCTURE-2` por completamente cerrada | `E:\Carpetas anbernic\psx\` | 🔴 pendiente, no bloqueante |
