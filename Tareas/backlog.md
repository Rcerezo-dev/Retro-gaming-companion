...
| CABLE-SYNC-NEWEST-CANON-1 | La dirección `newest` (línea ~939) compara `pc_index`/`ab_index` por ruta relativa cruda de **ambos** lados (ni siquiera aplica `canonical_rel_posix()` al lado PC antes de indexar, a diferencia de `pc_to_anbernic`) — el mismo problema de fondo que `CABLE-SYNC-SAVES-PREFIX-1/2` pero sin ninguno de los dos fixes aplicados. No comparte código con `_skip_existing_device`/`_skip_existing_pc` | `web/handlers/sync_cable.py:939-978` | 🔴 pendiente, sin decidir |
