...
**Paso 10 🚫 bloqueado por el harness**: `adb reboot` denegado por el clasificador
de auto-mode del entorno del agente ("Irreversible Local Destruction") — no es una
restricción del proyecto; pendiente que el usuario lo ejecute a mano y compruebe
`dumpsys activity services` sin abrir la app. **Pasos 2/3/4/7 ❌ bloqueados a mitad
de sesión, no por falta de funcionalidad**: tras dos ciclos de apagar/encender
pantalla durante las pruebas del Paso 8, el dispositivo volvió a mostrar un
lockscreen real (`input keyevent 4` no lo cerró, confirmando que es un keyguard,
no solo la notification shade) — mismo tipo de bloqueo que el Intento 1 original,
no automatizable sin gestos/bypass prohibidos. El switch "Sync automático" quedó
OFF toda la sesión (nunca se llegó a tocar). **Hallazgo nuevo de UI (bug, no
arreglado en esta sesión)**: `SettingsScreen.kt:66-69` — el `Column` raíz no tiene
`Modifier.verticalScroll(...)`; en el viewport real de la RG556 en landscape
(1920×1023 px tras las barras del sistema) el contenido desborda y el botón
"Sincronizar ahora", ambos switches y toda la sección "Historial de sync" quedan
fuera del área visible — solo se pudieron accionar los switches enviando toques
por coordenadas obtenidas de `uiautomator dump` (bounds recortados/poco fiables),
inviable para un usuario real con solo la pantalla táctil en este hardware.
Dispositivo quedó con Sync instantáneo ON (persistido, notificación activa), Sync
automático OFF, 3 filas en `sync_history` (todas `trigger=INSTANT`), archivos de
prueba y copias de depuración en `/sdcard` limpiados. Checklist actualizado paso a
paso en `Tareas/Validacion-Sync-Instantaneo-Historial.md` |
| ANDROID-SYNC-11 | `BootRestartReceiver` | 3 — Instantáneo | XS | 🟡 **reabierto e implementado 2026-09-25** — relanza `SyncForegroundService` tras `BOOT_COMPLETED` si `instantSyncEnabled` seguía activo. Permisos `FOREGROUND_SERVICE`/`FOREGROUND_SERVICE_DATA_SYNC`/`RECEIVE_BOOT_COMPLETED` (retirados en ANDROID-SYNC-12) devueltos al manifest con la justificación actualizada. Verificado con `./gradlew testDebugUnitTest`/`assembleDebug` reales (ambos en verde) — sin probar todavía reboot real en hardware |
