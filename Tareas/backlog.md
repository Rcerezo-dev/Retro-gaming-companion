...
| CATALOG-MATCH-REGION-2 | **Causa raíz encontrada 2026-09-04** (solo lectura, sin implementar — pendiente de rama propia): de los 289 casos PSX que `CATALOG-MATCH-REGION-1` no pudo resolver, 149 tienen serial leído del disco que no calza contra ningún `CatalogEntry.serial` candidato. Muestreadas 15 al azar: **14/15 son el mismo patrón** — el serial del Redump DAT trae sufijos que el disco real no tiene (`SLES-02997-0`/`SLES-12997-1` por disco de una copia múltiple, `SLUS-00067GHA`/`SCUS-94244CE` por variante "Greatest Hits"/"Collector's Edition", `SLES-01480-P-0` por país de impresión) — la comparación en `_match_by_title` (`catalog/matcher.py:328-332`, `_normalize_serial(h[0].serial) == real_norm`) exige igualdad exacta, así que `SLUS01067` (leído) nunca calza contra `SLUS01067GHA` (candidato) aunque sea literalmente el mismo disco. Un match por **prefijo** (`candidato.startswith(real_norm)` tras normalizar) habría resuelto 14/15 de la muestra, y de forma inequívoca en todos los casos muestreados (nunca dos candidatos distintos comparten el mismo prefijo). El 1/15 restante (`Front Mission 2 (Japan)`: detectado `SLPM87331`, candidatos `SLPS-01000`/`SLPM-87397` — ningún prefijo calza) es hueco real de cobertura del DAT, no bug de normalización. Los otros 140/289 siguen siendo el gap ya conocido del lector de disco (sin serial legible en absoluto, mismo patrón que `Tekken 3 (Japan) (Rev 1).cue`) | `catalog/matcher.py:328-332` (`_match_by_title`), `retroachievements/ra_hash_psx.py:detect_psx_boot_serial` | 🔴 pendiente de implementar (causa raíz clara: cambiar `==` a `startswith` tras normalizar) |
