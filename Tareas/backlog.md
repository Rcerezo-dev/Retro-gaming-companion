...
| ARCADE-DAT-CONTAMINATION-7 | **Hecho 2026-09-03**: hallazgo de `DECOMPRESS-ARCADE-GAP-4` — 190 ZIPs intactos + 203 carpetas de residuo suelto (206 detectadas, 3 resultaron ser la raíz completa de `atari5200/`/`intellivision/`/`wonderswan/` — no se aislaron enteras, solo los 57 ficheros sueltos individuales que votaban arcade en esa raíz, para no arrastrar contenido legítimo) aislados en `E:\Carpetas anbernic\_ARCADE_MISFILED_STAGING\` (fuera de `Unknown/`, ver -8) y pasados por `organize-source --apply`: **190/190 ZIPs reclasificados** a `arcade/` sin incidentes (incluye `psx__Raystorm (Japan).zip` — confirma la corrección de `DECOMPRESS-ARCADE-GAP-1` de que el set base `raystorm` sí existe, solo estaba mal etiquetado), 26 conflictos de nombre sin resolver (ficheros sueltos tipo `1.bin`/`7.bin` con nombre duplicado en destino). Los 203 residuos sueltos NO se pudieron reclasificar por esa vía (`organize-source` solo reconoce arcade por CRC en ZIPs intactos, no en ficheros sueltos sin contenedor) — verificados aparte con un script de sesión que recalcula CRC real y busca el set en `arcade/`: **134/242 items con backup 100% íntegro confirmado → descartados** a `_ARCADE_MISFILED_STAGING\_descartados\` (papelera, recuperable). Quedan sin resolver: **60 sin ningún ZIP del set en `arcade/`** (mismo patrón que los clones `raystormj/o/u` de ayer — puede que el set base tampoco exista, o falte re-descargar), **22 con el set presente pero cobertura parcial** (probable repack que mezcla varios sets, como `Ryujin`), **26 carpetas vacías** (residuo del proceso de aislamiento, sin contenido real, limpieza trivial pendiente) | `E:\Carpetas anbernic\_ARCADE_MISFILED_STAGING\` | ✅ auditoría + reclasificación ZIPs; 🔴 pendiente triage de 82 sets sin backup confirmado + 26 carpetas vacías |
...
| ARCADE-DAT-CONTAMINATION-9 | Repetir la auditoría completa de `DECOMPRESS-ARCADE-GAP-4`/`ARCADE-DAT-CONTAMINATION-7` contra `H:\ROMs` — no se pudo hacer el 2026-09-03 porque el lector SD no estaba conectado | `H:\ROMs\` | 🔴 pendiente, requiere conectar el lector SD |
...
| ID | Task | Archivo(s) | Estado |
|----|------|-----------|--------|
| CATALOG-MATCH-BUG-1 | Investigado, causa raíz confirmada: `_match_by_title()` no filtra por plataforma cuando la extensión es ambigua (no hay contexto de carpeta disponible en `match()`) — arreglo real requiere pasar la carpeta/ruta hasta el matcher y usar `PLATFORM_BY_FOLDER` como narrower adicional cuando `ext_platform` es `None`. Alcance: 428 ficheros confirmados fuera de `arcade/`, 807 más dentro de `arcade/` sin auditar en detalle. Sin implementar el fix todavía (rama propia) | `catalog/matcher.py:196-233` (`match()`), `catalog/matcher.py:235-283` (`_match_by_title()`, filtro roto en 258-263) | 🔴 pendiente, alcance confirmado — requiere pasar contexto de carpeta a `match()` |
...
| DEVPROFILE-4 | Manifiesto Tier A + backup del perfil al remoto — **alcance recortado tras DEVPROFILE-0**: `config/<core>/*.cfg`, `retroarch-core-options.cfg`, `config/remaps/`, `autoconfig/`, shaders, `.opt` en bulk, BIOS/`system/`; `retroarch.cfg` solo en el lado PC. Standalones de PC sin cambio: `duckstation/settings.ini`, `PCSX2/inis/`, `Dolphin/Config/`, `es_systems.cfg` | M | 🟡 backend hecho 2026-09-01 (`services/device_profile.py`: `detect_tier_a_sources`, `export_profile_sources`/`import_profile_sources`, 5 tests) — falta la pantalla de Settings "Perfil del dispositivo" para confirmar las fuentes detectadas. Rama `feature/devprofile-3-4-path-tokenizer-manifest`. Ver Roadmap §5 |
| DEVPROFILE-5 | Botón PC — `rommgr restore`: `download-tools.ps1` → `config.toml` desde el perfil (sustituye media `wizard.py`) → Tier A con rutas reescritas → regenerar Tier B → `bios_checker` y reportar lo que falta | M | ⬜ diseñado 2026-09-01, ver `Tareas/Roadmap-DEVPROFILE-5-6.md` — encontró que falta 5a (subir el manifiesto al remoto, hueco real de DEVPROFILE-4 sin cerrar) antes de poder implementar el resto. 4 preguntas abiertas para el usuario en §5 antes de codear |
...
| GBA-MISPLACED-2 | **Causa raíz, no arreglada**: `.zip`/`.chd`/`.iso` no están en `PLATFORM_BY_EXTENSION` (`detection/platforms.toml`, sección `[ambiguous]`, correcto por diseño — un `.chd` puede ser PSX/Saturn/Dreamcast/arcade) — cuando el fallback de título (`_match_by_title`, `catalog/matcher.py:234`) encuentra el mismo título en varias plataformas del DAT y la extensión no desambigua (MATCH-FIX-2, `catalog/matcher.py:257`), se queda con `hits[0]` (orden arbitrario de carga de los `.dat`) en vez de usar la carpeta real del archivo como señal de desempate. Esto es lo que corrompió las 43 filas de GBA-MISPLACED-1a/1b — puede seguir pasando en cualquier future `rommgr match`/scan de archivos con extensión ambigua cuyo título coincida por casualidad con otro juego de otra plataforma. Arreglo propuesto (no implementado): cuando `ext_platform` es `None` (extensión ambigua) y hay >1 hit, usar la carpeta contenedora del archivo (`detect_platform(Path(filename))`, ya fiable, mismo mecanismo usado para corregir hoy) como criterio de desempate antes de caer a `hits[0]` | `catalog/matcher.py:250-270` (`_match_by_title`, MATCH-FIX-2) | 🔴 causa raíz identificada, sin arreglar — recurrirá si no se corrige |
