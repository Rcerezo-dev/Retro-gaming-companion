...
| AUD-2 | **Verificación post-transferencia** — hash origen/destino tras cada push/pull (`adb shell md5sum`); si difiere, no propagar y reportar; columna `verified` en `save_sync_log` | Sync | S-M | 🟡 rama `feature/aud-2-transfer-verify` (apilada sobre AUD-1) — verificación en el chokepoint compartido `AdbTransport.pull/push(verify=True)`: pull baja a `.part` y solo sustituye el destino si el MD5 coincide (mismatch → `AdbVerifyError`, el save bueno queda intacto); push compara el MD5 remoto tras subir. Si `md5sum` no existe en el device, no bloquea (`last_verified=None`). La heredan el cable sync manual (solo saves, no ROMs) y el auto-sync daemon (siempre). Columna `verified` en `save_sync_log` (migración en `schema.py`, verificada sobre la BD real) + las transferencias ADB de saves ahora se registran en esa tabla (alimenta el panel de AUD-1). Contadores `verified`/`verify_failed` en el resultado del job y en la UI. 8 tests (671 pass). **Falta**: corrupción real solo simulada con transporte fake — validar con consola (V-AUD-2) |
