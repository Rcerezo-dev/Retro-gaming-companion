...
| ID | Task | Archivo(s) | Estado |
|----|------|-----------|--------|
| CABLE-SYNC-DOWNLOAD-DEST-1 | `_adb_copy_to_pc()` escribe en `pc_root / rel_posix` usando la ruta **cruda** del dispositivo (`info.android_path.removeprefix(android_prefix)`), sin canonicalizar — a diferencia de `pc_to_anbernic` (que sí aplica `canonical_rel_posix()` antes de subir). Un archivo genuinamente nuevo descargado desde `saves/gba/foo.sav` aterriza en `pc_root/saves/gba/foo.sav`, no en la ubicación D2 canónica — mezcla convenios en el PC en vez de organizarlos. El fix de `CABLE-SYNC-SAVES-PREFIX-2` evita el síntoma práctico (ya no se re-descargan archivos que ya existen), pero no corrige el destino de los que sí son nuevos | `web/handlers/sync_cable.py:596` (`_adb_copy_to_pc`), línea 914 (llamada en `anbernic_to_pc`) | 🔴 pendiente, sin decidir — cambia el destino de descargas para todos los usuarios, no solo el caso Wii/saves, requiere decisión de diseño propia |
...
| CABLE-SYNC-NEWEST-CANON-2 | Mismo problema de fondo (comparación por ruta cruda sin canonicalizar) en las otras 2 reimplementaciones independientes de la lógica "newest" encontradas al arreglar `CABLE-SYNC-NEWEST-CANON-1`: (a) `sync/cable_engine.py:plan_direction()` — compartido por el sync manual en modo filesystem/SD montada y el daemon SD-auto (`web/cable_sync_daemon.py` línea ~574); (b) el fallback "RetroArch (legacy)" del daemon auto-sync ADB (`web/cable_sync_daemon.py` líneas ~425-459, activo cuando `get_adb_sync_sources()` no devuelve fuentes — usuarios sin `emulator_paths` configurado). Ninguna de las dos comparte código con `sync_cable.py`, así que el fix de hoy no las cubre. El resto de rutas per-emulador del daemon (`adb_sources` con `local_saves`/`android_saves` dedicados) NO tienen este problema — son carpetas 1:1 sin ambigüedad de plataforma | `sync/cable_engine.py` (`plan_direction`, rama `newest`), `web/cable_sync_daemon.py` (fallback "RetroArch (legacy)") | 🔴 pendiente, sin decidir — descubierto pero no investigado en profundidad (sin mapeo real de cuántos usuarios activan cada rama) |
