...
## Resumen de lo completado en el Día 16 (sesiones A y B — 2026-03-20/21)

> Detalle completo en `Tareas/diario/día16.md`.

**Sesión A (2026-03-20):**
- S34d-0: fix `parse_bins_from_cue` sin comillas (Street Fighter Collection y otros)
- S34d-1: nueva pestaña "Formatos de archivo" en la UI
- BUG-5: observabilidad CHD, protección arcade, verificación ES-DE/PSX
- BUG-6 (B6-1..B6-6): bugs en emuladores, scraper, inbox, RetroArch

**Sesión B (2026-03-21):**
- B7-1..B7-9: 9 bugs de UI detectados en uso real (RA tabla, Library Doctor, Cable Sync, Inbox, MAME, N64, logs)
- B6-7: campos media en gamelist.xml + documentación temas ES-DE
- Corrección ES-DE: exit code 1 al lanzar juegos → wrappers `.bat`
- Fix "no se han encontrado archivos de juego" → extensiones `.bin`/`.img` eliminadas
- Limpieza pipeline de exportación gamelists
- `move_disc_set_to_subfolder()` — organización PSX/Saturn/PS2/Dreamcast/GC/Wii en subcarpetas
- Portabilidad: `config.toml.example`, `Tareas/` en git