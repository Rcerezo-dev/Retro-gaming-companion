...
| B0-3c | RG556: `/storage/emulated/0/RetroArch/config/` vía ADB — mismo prefijo ya validado en producción para `saves`/`states` de este dispositivo (`config.py` `EMULATOR_MAP`) | Sondeo ADB, igual que `anbernic_root` | ✅ 2026-08-18 — `_detect_android_ra_config_dir()` en `web/handlers/config.py`, mismo formato de respuesta que `_detect_retroarch_install()` (PC). Candidato: `<auto_sync_android_path>/config`; confirmado con `AdbTransport.test_path()` (ya existente, reutilizado sin ADB shell nuevo — mismo método que ya usa `/api/test-adb-path`) en vez de asumir que está ahí. Endpoint `GET /api/detect-android-ra-config-dir`, resuelve el dispositivo conectado con `resolve_single_device_transport()` (mismo helper ya usado en `collection.py` para overrides). 4 tests nuevos (`tests/test_detect_android_ra_config_dir.py` + handler-level). Verificado contra el servidor real (sin RG556 a mano: confirma el mensaje "conecta el dispositivo" correcto, la ruta candidata contra un dispositivo real queda para B0-3d/CFG-PORGAME-10) |
| B0-3d | Botón "🔍 Detectar automáticamente" en Settings, junto a los campos `retroarch_path`/`ra_config_dir`/Android | Mismo patrón UX ya usado en la app (ADB devices, LAN IP) | ✅ 2026-08-18 — el de PC ya existía y ahora también rellena `ra_config_dir`. El de Android no rellena ningún campo (no hay uno persistido — la ruta candidata sale de `auto_sync_android_path`, ya guardado): botón "🔍 Detectar carpeta config de RetroArch (Android)" junto al campo "Ruta Android (RetroArch)" en Cable Sync (`tab-cable.html`), llama a `GET /api/detect-android-ra-config-dir` (B0-3c) y muestra el resultado (`detectAndroidRaConfigDir()`, `sync.js`) — mismo patrón `_txtCls`/resultado inline que `detectAdbDevices()` |

### B1 — Diseño

| ID | Task | Esfuerzo | Estado |
|----|------|----------|--------|
| CFG-PORGAME-1 | Diseño de la auto-detección de rutas (B0-3a..d) — orden de fallback PC, sondeo ADB Android, qué pasa si no se encuentra nada (pedir al usuario, no bloquear) | S | ✅ 2026-08-18 — mitad PC resuelta al implementar CFG-PORGAME-5 directamente (el fallback ya existía). Mitad Android resuelta al implementar B0-3c: un único candidato (`<auto_sync_android_path>/config`, no una lista de fallback — a diferencia de PC no hay varias instalaciones posibles), confirmado vía ADB en vez de asumido; si no se encuentra o no hay dispositivo, se devuelve el motivo sin bloquear nada (el resto de la app sigue funcionando, solo el lado Android de overrides queda vacío) |
| CFG-PORGAME-2 | Documentar formato final `<rom>.opt` y dónde vive por core/plataforma (base: `docs.libretro.com/guides/overrides/` + estructura ya usada por RetroArch) | XS | ✅ 2026-08-18 — nueva sección "Overrides por juego en Retro Vault" en `docs/architecture/platforms-cores.md`: los 3 niveles de especificidad de RetroArch (core/carpeta/juego, solo el último es el que gestiona la app), formato de texto plano con ejemplo real, ruta por plataforma (PC vs Android), y por qué el nombre de core importa para poder copiar (enlaza con `SHARED_CORES` de CFG-PORGAME-3/8) |
...
| ID | Task | Esfuerzo | Estado |
|----|------|----------|--------|
| CFG-PORGAME-5 | Backend: auto-detección de rutas | M | ✅ mitad PC (B0-3a/b) — 2026-08-17. ✅ mitad Android (B0-3c) — 2026-08-18. Solo falta B0-3d (botón en Settings, UI pura) |
