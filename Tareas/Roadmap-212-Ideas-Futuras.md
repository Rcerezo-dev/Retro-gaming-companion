...
| B0-3d | Botón "🔍 Detectar automáticamente" en Settings, junto a los campos `retroarch_path`/`ra_config_dir`/Android | Mismo patrón UX ya usado en la app (ADB devices, LAN IP) | ✅ el de PC ya existía y ahora también rellena `ra_config_dir`. ⬜ falta el de Android — el backend ya está listo (B0-3c, `GET /api/detect-android-ra-config-dir`), solo falta el botón + JS en Settings |
