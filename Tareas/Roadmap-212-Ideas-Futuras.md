...
| CFG-PORGAME-3 | Diseño de "copiar a [PC\|Android]" por juego — **no** reutiliza `sync.ra_config_dir`/`ra_config_remote` (eso es respaldo de la config global completa, otro caso de uso); acción explícita, un juego a la vez, deshabilitada si el core no coincide entre PC y RG556, con aviso de rendimiento cuando sí coincide | S | ⬜ |
...
| CFG-PORGAME-9 | Frontend: UI del editor + botón de copia puntual con aviso de rendimiento | M | 🟡 mitad editor hecha junto con CFG-PORGAME-7 (2026-08-18, panel Overrides en Colección). Falta el botón "Copiar a [PC\|Android]" — depende de CFG-PORGAME-3 (diseño, ⬜) y CFG-PORGAME-8 (backend copia, ⬜) |
